 ...
 */
package br.com.personality.insights.helper;
...
public class Twitter4JHelper implements RateLimitStatusListener {
  ...
  public List<Status> getTweets(String idOrHandle, Set<String> langs, int numberOfNonRetweets) throws Exception {
    ...
    if (idOrHandle.startsWith("@")) {
      ...
      if (user == null)
        throw new Exception("Handle " + idOrHandle + " não é um identificador de twitter válido.");
      userId = user.getId();
    } else {
    ...
    do {
      ...
      Paging page = new Paging(cursor, numberOfNonRetweets);
  ...
  private void checkRateLimitAndThrow() throws Exception {
    if (isRateLimited()) {
      ...
      throw new Exception("O limite de taxa de API do Twitter foi atingido."
        + "Nenhum pedido será enviado até que o limite de taxa seja redefinido em " + dateText);
    }
  }
  ...
  public void onRateLimitReached(RateLimitStatusEvent rlStatusEvent) {
    ...
    System.err
        .println("Limite de taxa do Twitter atingido, interrompendo todas as solicitações de " + rls.getSecondsUntilReset() + " seconds");
  }

  /**
   * No status do limite de taxa.
   *
   * @param rlStatusEvent the rl status event
   */
  @Override
